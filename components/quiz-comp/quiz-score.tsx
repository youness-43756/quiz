...
import { scoreTypes } from '@/context/contextProvider'
...
export default function QuizScore({ replay, score }:
    ...
    return (
        <div className="flex flex-col items-center gap-8">
            ...
            {
                score.points === 0 ? <Angry className={scoreEmojiStyle} /> :
                    score.points < 4 ? <Frown className={scoreEmojiStyle} /> :
                        score.points < 7 ? <Meh className={scoreEmojiStyle} /> :
                            score.points < 10 ? <Smile className={scoreEmojiStyle} /> :
                                <Flame className={scoreEmojiStyle} />
            }
